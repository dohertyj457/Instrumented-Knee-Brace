...
//Sensor_data_collectionV2.ino
//UVM SEED Team 15: Instrumented Knee Brace
//Created 2/23/2021 from previous drafts in Fall 2020
  //V2 2-26-21: addition of potentiometer function (commented out in loop), increase baud rate
...
int times[4]; //***change to 5 after addition of pot 

//Arduino communication
int SDPin = 10; //Pin SD Reader is connected to
int PotPin = A2; //Pin potentiometer is connected to 
...
void setup() {
  ...
  Serial.begin(500000); // *changed baud rate from 9600 to 500000
  Wire.begin();
  setupMPU();
  setupSD();
}
...
float GetPot(){
  ...
  float pot = map(sensorValue,0,1023,0,255);
  // print out the value you read:
  //Serial.println(pot);
  // delay(10);    
  return pot;
...
void writeIMU(long time1, long time2){
  ...
  myFile.print(times[1]);myFile.print(":");myFile.print(rotX1); myFile.print(","); myFile.print(rotY1); myFile.print(","); myFile.print(rotZ1); myFile.print(","); 
  myFile.print(gForceX1); myFile.print(","); myFile.print(gForceY1); myFile.print(","); myFile.print(gForceZ1); myFile.print(","); 
  myFile.print(times[2]);myFile.print(":");myFile.print(rotX2); myFile.print(","); myFile.print(rotY2); myFile.print(","); myFile.print(rotZ2); myFile.print(",");
...
void writeEMG(long time3, long time4){
  ...
  //myFile.println(sigtitle1);
  myFile.print(times[3]); myFile.print(":"); myFile.print(voltage1); myFile.print(","); myFile.print(times[4]); myFile.print(":"); myFile.println(voltage2); // this may need to be adjusted when printing more than one signal 
  //close the file:
  myFile.close();
  }
  else {
...
void writePot(long time5){
  //print data onto SD Module
  myFile = SD.open("POTtest.txt", FILE_WRITE);
  // if the file opened okay, write to it:
  if (myFile) {
  //Serial.print("POTtest.txt...");
  //myFile.println(sigtitle1);
  myFile.print(times[5]); myFile.print(":"); myFile.print(pot); 
  //close the file:
  myFile.close();
  }
  else {
